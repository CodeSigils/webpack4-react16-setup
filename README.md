...
* By default Jest will create a `__test__` folder will include test files according this regex pattern: `(/__tests__/.*|\\.(test|spec))\\.(js|jsx)$`. If we want a custom one, we create a `jest.config.js` file inside the folder of our choice. In this case the `specs` folder:
